 ...
 */
void BufferedWriter::putHex(unsigned long n) {
  size_t len;
  if (n < 0X10000) {
 ...
 */
void BufferedWriter::putNum(signed char n) {
  if (n < 0) {
    putChar('-');
    n = -n;
  }
 ...
 * \param[in] n number to be written
 */
void BufferedWriter::putNum(unsigned char n) {
 ...
 * \param[in] n number to be written
 */
void BufferedWriter::putNum(signed short n) {
  if (n < 0) {
    putChar('-');
    n = -n;
  }
 ...
 * \param[in] n number to be written
 */
void BufferedWriter::putNum(unsigned short n) {
  if (n < 256) {
    putNum((unsigned char)n);
    return;
  }
 ...
 * \param[in] n number to be written
 */
void BufferedWriter::putNum(signed long n) {
  if (n < 0) {
    putChar('-');
    n = -n;
  }
 ...
 * \param[in] n number to be written
 */
void BufferedWriter::putNum(unsigned long n) {
  uint8_t len;
  if (n < 0X10000) {
    putNum((unsigned short)n);
    return;
...
void BufferedWriter::putStr_P(PGM_P str) {
  for (char c; (c = pgm_read_byte(str)); str++) putChar(c);
}
//------------------------------------------------------------------------------
